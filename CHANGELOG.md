...
- Local Kubernetes deployment support with `k3d` runbook, `k8s/` manifests (web/api/postgres/ingress), and migration job for in-cluster schema setup
- Web production container (`Dockerfile.web`) with Nginx SPA routing for Kubernetes deployment
